    ...
    if 0:
        try:
            ...
            if xpu == "gpu":
                ...
                model = AutoModel.from_pretrained(model_name_local, trust_remote_code=True).half().cuda()
                model = model.eval()
            elif xpu == "cpu":
                
                if model_name == "chatglm2-6b":
                    stm = time.time()                    
                    model = ChatGLM(model_all_local_path + "\\ggml-chatglm2-6b-q4_0.bin", n_threads=20,n_ctx=4096) # use_mmap=True, n_threads=20, n_ctx=512
                    print("model load time (s)= ", time.time() - stm)
                else:
                    tokenizer = AutoTokenizer.from_pretrained(model_name_local, trust_remote_code=True)
